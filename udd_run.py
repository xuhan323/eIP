...
def UDD_MD_run(atoms, Runtime, PATH, Temp, sigma_cutoff, filename, sampling=20, dt=0.5, tau_t=20, intervals=1, A=1000, B=10, C=10, D=10, log_dir="./Results/"):
    ...
    model = PainnModel(num_interactions=4, hidden_state_size=256, cutoff=6.0, pdb=True, A=A, B=B, C=C, D=D, MD=True)
    model.to(device)
    ...
    def printenergy(a=atoms):
        ...
        energy = a.calc.get_property('origin_energy')
        ...
        with open(savedir + "log." + f"A_{A}_B{B}_C{C}_D{D}" + str(Temp), 'a') as f:
            f.write(
                f"Steps={steps:8.0f} Uncer={uncer_mean:8.2f} {uncer_std:8.2f} Epot={energy:8.2f} force_bias={for_b_max:8.2f} {for_b_m:8.2f} {for_b_s:8.2f} temperature={temp:8.2f}\n")
    ...
    # dyn.attach_trajectory(filename, interval=2)
    ...
    try:
        ...
        dyn.attach(sample, interval=intervals)
        dyn.run(Runtime)
        ...
        # 确保轨迹文件被正确关闭
        traj.close()
        print(f"Trajectory file closed. Total steps: {steps}")

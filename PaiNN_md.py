...
class PainnModel(nn.Module):
    ...
    def __init__(
        ...
        target_mean=[0.0],
        target_stddev=[1.0],
        atomwise_normalization=True, MD=False, A=0, B=0,C=0,D=0,
        **kwargs, 
    ):
        super().__init__()
    ...
    def forward(self, input_dict, compute_forces=True):
        ...
        if self.MD:    
            ...
            epistemic_uncertainty = torch.sqrt((f_beta)/(f_v*(f_alpha-1) + 10e-6))

            sigma = torch.mean(epistemic_uncertainty)

            energy_total = energy + self.A * torch.exp(-(sigma/(input_dict.natoms*self.B))) - self.A

            energy_bias = energy_total - energy

            force_bias = -grad(outputs=energy_bias, inputs=input_dict.pos, grad_outputs=torch.ones_like(energy_bias),create_graph=True,retain_graph=True)[0]
            # force_biased = torch.clamp(force_bias, min=-0.3, max =0.3)
            sig=self.C
            force_bias_gaussian = force_bias * self.D* torch.exp(-force_bias  ** 2 / (2 * sig ** 2)) / (2 * torch.pi * sig ** 2)
            force_biased = force_bias_gaussian

            result_dict = {'energy': energy_total}
            result_dict['energy_orig'] = energy
            result_dict['uncertainty'] = epistemic_uncertainty
            result_dict['energy_biased'] = energy_bias - energy
            result_dict['forces_orig'] = force
            result_dict['forces_biased'] = force_biased
            result_dict['forces'] = force_biased + force

            return result_dict
        
        return energy, energy,node_para